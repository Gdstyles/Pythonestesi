...
SELECT a.folio, a.fecha, a.dias, a.valor_dia, c.nombre, c.rut FROM arriendo AS a JOIN cliente AS c ON a.rut_cliente = c.rut;

-- 2. Listar los clientes sin arriendos. 

SELECT c.nombre FROM cliente AS c LEFT JOIN arriendo AS a ON c.rut = a.rut_cliente WHERE a.rut_cliente IS NULL;


-- 3. Liste RUT y Nombre de las tablas empresa y cliente. 

SELECT rut, nombre FROM cliente UNION SELECT rut, nombre FROM empresa;

-- 4. Liste la cantidad de arriendos por mes. 


SELECT EXTRACT(MONTH FROM fecha) AS mes, COUNT(folio) AS cantidad_arriendos FROM arriendo GROUP BY mes;