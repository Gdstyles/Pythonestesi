...
SELECT V.* FROM vehiculo v LEFT JOIN venta ven ON v.id_vehiculo = ven.id_vehiculo WHERE ven.id_vehiculo IS NULL;

-- sin alias = SELECT * FROM vehiculo LEFT JOIN venta ON id_vehiculo = id_vehiculo WHERE id_vehiculo IS NULL;
...
SELECT ven.folio, ven.fecha, ven.monto, c.nombre, c.rut, v.patente, v.modelo, m.nombre FROM venta ven JOIN cliente c ON ven.rut_cliente = c.rut JOIN vehiculo v ON ven.id_vehiculo = v.id_vehiculo JOIN marca m ON v.id_marca = m.id_marca WHERE EXTRACT(MONTH FROM ven.fecha) = '1' AND EXTRACT(YEAR FROM ven.fecha) = 2020;
...
SELECT m.nombre AS marca, SUM(ven.monto) AS total_ventas FROM venta ven JOIN vehiculo v ON ven.id_vehiculo = v.id_vehiculo JOIN marca m ON v.id_marca = m.id_marca WHERE EXTRACT(YEAR FROM ven.fecha) = 2020 GROUP BY marca;
...
SELECT rut, nombre FROM cliente UNION rut, nombre FROM empresa;

SELECT c.rut, c.nombre FROM cliente c CROSS JOIN empresa e;