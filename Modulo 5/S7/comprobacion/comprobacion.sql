...
SELECT nombre FROM cliente WHERE rut NOT IN (SELECT rut_cliente FROM venta);
...
-- tabla cliente c = c.nombre, c.rut 
 ...
 SELECT ven.folio, ven.fecha, ven.monto, c.nombre, c.rut FROM venta ven INNER JOIN cliente c ON ven.id_cliente = c.id_cliente;
...
SELECT 
    ...
    monto,
    CASE 
        WHEN monto <= 1000000 THEN 'Standar'
        WHEN monto > 1000000 AND alta <= 3000000 THEN 'Gold'
...
SELECT v.*, m.nombre FROM venta AS vehiculo
